...
"""
    sample_next_points(ϕ_pred, current_ϕs; strategy="OPS")
...
- `current_ϕs`: 当前已经被 Black-box 评估过的自变量点集合 (1维时通常是排序好的 array，多维时则是点集数组)。
- `strategy`: 采样策略，支持 "OPS"(On-Position), "BS"(Bisection), "TS"(Tri-section), "QS"(Quad-section), "RS"(Random)。

返回:
- 一个数组，包含需要进行下一轮评估的全新的坐标点。
"""
function sample_next_points(ϕ_pred, current_ϕs; strategy="OPS")
    ...
    if isempty(valid_preds)
        ...
        return _fallback_random(current_ϕs)
    end
    ...
    if strategy == "OPS"
        ...
        for vp in valid_preds
            if !_is_too_close(vp, current_ϕs, 1e-7)
                push!(new_points, vp)
            end
        end
        return isempty(new_points) ? _fallback_random(current_ϕs) : new_points
    end
    ...
    if !is_1d
        ...
        return sample_next_points(ϕ_pred, current_ϕs, strategy="OPS")
    end
    ...
    for p_val in valid_preds
        ...
        # 这里假设 current_ϕs 在 1D 时是排序好的标量数组
        idx = searchsortedfirst(current_ϕs, p_val)
        
        # 边界处理：如果预测点落出外推区域，回退为 OPS
        if idx == 1 || idx > length(current_ϕs)
            if !_is_too_close(p_val, current_ϕs, 1e-7)
                push!(new_points, p_val)
            end
            continue
        end

        phi_i = current_ϕs[idx - 1]
        phi_j = current_ϕs[idx]
        interval_len = phi_j - phi_i
        
        if strategy == "BS" # Bisection (中间加1点)
            pt = phi_i + interval_len / 2.0
            if !_is_too_close(pt, current_ϕs, 1e-7)
                push!(new_points, pt)
            end
    ...
    if isempty(new_points)
        new_points = sample_next_points(ϕ_pred, current_ϕs, strategy="OPS")
    end
...
# 内部辅导函数
function _is_too_close(pt, ϕ_hist, tol=1e-7)
    if isempty(ϕ_hist)
        return false
    end
    if pt isa Number
        # 1D 系
        return minimum(abs.(ϕ_hist .- pt)) < tol
    else
        # 2D 或 多维
        # 计算欧式距离
        dists = [sqrt(sum((x .- pt).^2)) for x in ϕ_hist]
        return minimum(dists) < tol
    end
end

function _fallback_random(current_ϕs)
    new_points = similar(current_ϕs, 0)
    if isempty(current_ϕs)
        push!(new_points, rand())
        return new_points
    end
    
    if current_ϕs[1] isa Number
        # 1D 均匀产生一个
        push!(new_points, rand() * (maximum(current_ϕs) - minimum(current_ϕs)) + minimum(current_ϕs))
    else
        # 高维：在数据的包围盒中生成一个
        mins = [minimum(v) for v in eachrow(hcat(current_ϕs...))]
        maxs = [maximum(v) for v in eachrow(hcat(current_ϕs...))]
